...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  const addProdutoCarrinho = async () => {
    saveCartID(id);
    const requisicaoFunc = await fetchProduct(id);
    const armazenaFunc = createCartProductElement(requisicaoFunc);
    const recebeClass = document.querySelector('.cart__products');
    recebeClass.appendChild(armazenaFunc);
  };
  section.addEventListener('click', addProdutoCarrinho);
